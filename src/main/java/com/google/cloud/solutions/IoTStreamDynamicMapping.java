...
/**
 * This class defines the Apache Beam pipeline that consumes Cloud IoT Core event messages and stores them in BigQuery.
 * This pipeline
 * 1. parses each incoming message against a set of configured data schemas
 * 2. transforms the data to the configured BigQuery table format using configured sch
 * 3. stores the data to the configured BigQuery table
 *
 * The incoming messages that can not be validate against the configured data schemas are stored in an unknown message table in BigQuery
 */
public class IoTStreamDynamicMapping {
    ...
    /**
     * Reads the IoT event stream from PubSub topic and branches the stream into
     * - a validated message stream
     * - an unknown message stream
